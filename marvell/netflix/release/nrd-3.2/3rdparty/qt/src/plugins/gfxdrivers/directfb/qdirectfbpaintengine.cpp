...
public:
    ...
    void blit(const QRectF &dest, IDirectFBSurface *surface, const QRectF &src, bool alpha);
                                     ...
                                     Qt::ImageConversionFlags flags)
{
    ...
    */
    ...
    const bool hasAlpha = QDirectFBScreen::hasAlphaChannel(imgSurface);
    d->prepareForBlit(hasAlpha);
    CLIPPED_PAINT(d->blit(r, imgSurface, sr, hasAlpha));
    if (release) {
#if (Q_DIRECTFB_VERSION >= 0x010000)
        d->surface->ReleaseSource(d->surface);
#endif
                                      ...
                                      const QRectF &sr)
{
    ...
    if (pixmap.pixmapData()->classId() != QPixmapData::DirectFBClass) {
        ...
        if (!(d->compositionModeStatus & QDirectFBPaintEnginePrivate::PorterDuff_Supported)
            ...
            const bool hasAlpha = pixmap.hasAlphaChannel();
            d->prepareForBlit(hasAlpha);
            IDirectFBSurface *s = dfbData->directFBSurface();
            CLIPPED_PAINT(d->blit(r, s, sr, hasAlpha));
        }
    }
}
...
void QDirectFBPaintEngine::fillRect(const QRectF &rect, const QBrush &brush)
{
    ...
    if (d->clipType != QDirectFBPaintEnginePrivate::ComplexClip) {
        ...
        case Qt::TexturePattern: {
...
#if 0
            const QRect transformRect = stateTransform.mapRect(rect);
            QTransform transform(stateTransform);
#else
            const QRect transformRect = rect.toRect();
            ...
            QPointF offset = rect.topLeft() - brushOrigin;
            CLIPPED_PAINT(d->drawTiledPixmap(transformRect, texture, offset, transform));
            return; }
        default:
            break;
        }
...
void QDirectFBPaintEnginePrivate::setDFBColor(const QColor &color)
{
...
#if 1
    if( q->state()->composition_mode == QPainter::CompositionMode_Clear ) {
...
#endif
    {
        ...
        surface->SetDrawingFlags(surface, alpha == 255 &&
                                 !(compositionModeStatus & PorterDuff_AlwaysBlend) ? DSDRAW_NOFX : DSDRAW_BLEND);
    }
}
...
void QDirectFBPaintEnginePrivate::blit(const QRectF &dest, IDirectFBSurface *s, const QRectF &src, bool alpha)
{
    Q_UNUSED(alpha);
...
#ifdef QT_DIRECTFB_MTK
        if( alpha ) {
            const DFBRectangle dRect = { 0, 0, dr.width(), dr.height() };
            static IDirectFBSurface *tmpSurface = 0;
            int req_width = dr.width(), req_height = dr.height();
            if( tmpSurface ) {
                int w, h;
                tmpSurface->GetSize(tmpSurface, &w, &h);
                if( w < req_width || h < req_height ) {
                    if( w > req_width )
                        req_width = w;
                    if( h > req_height )
                        req_height = h;
                    QDirectFBScreen::instance()->releaseDFBSurface(tmpSurface);
                    tmpSurface = 0;
                }
            }
            if( !tmpSurface  )
            {
                tmpSurface = QDirectFBScreen::instance()->createDFBSurface(QSize(req_width, req_height),
                                                                           QImage::Format_ARGB32,
                                                                           QDirectFBScreen::TrackSurface);
                tmpSurface->SetDrawingFlags(tmpSurface, DSDRAW_NOFX);
                tmpSurface->SetBlittingFlags(tmpSurface, DSBLIT_NOFX);
                tmpSurface->SetPorterDuff(tmpSurface, DSPD_SRC);
            }
            result = tmpSurface->StretchBlit(tmpSurface, s, &sRect, &dRect);
            if (result == DFB_OK)
                result = surface->Blit(surface, tmpSurface, &dRect, dr.x(), dr.y());
        } else
#endif
        {
                                                  ...
                                                  const QPointF &off, const QTransform &pixmapTransform)
{
    ...
    if (transform.isScaling() || pixmapTransform.isScaling()) {
        ...
        const QSize mappedSize(ceil(pixmapSize.width() * pixmapTransform.m11()),
                               ceil(pixmapSize.height() * pixmapTransform.m22()));

        QPointF dstPt(destinationRect.topLeft());
        dstPt.rx() = fixCoord(dstPt.x(), mappedSize.width(),  qreal(off.x())-pixmapTransform.m31());
        dstPt.ry() = fixCoord(dstPt.y(), mappedSize.height(), qreal(off.y())-pixmapTransform.m32());
        dstPt.rx() = qFloor(dstPt.x());
        dstPt.ry() = qFloor(dstPt.y());

        for(int y = dstPt.y(); y <= destinationRect.bottom(); y += mappedSize.height()) {
            qreal x = dstPt.x();
            while (x <= destinationRect.right()) {
                const DFBRectangle destination = { x, y, mappedSize.width(), mappedSize.height() };
        ...
        QPointF dstPt(destinationRect.topLeft());
        const QPointF off_inv = pixmapTransform.inverted().map(off);
        dstPt.rx() = fixCoord(dstPt.x(), pixmapSize.width(),  off_inv.x());
        dstPt.ry() = fixCoord(dstPt.y(), pixmapSize.height(), off_inv.y());
        dstPt.rx() = qFloor(dstPt.x());
        dstPt.ry() = qFloor(dstPt.y());

        int horizontal = qMax(1, destinationRect.width() / pixmapSize.width()) + 1;
        if (dstPt.x() != destinationRect.x())
            ++horizontal;
        int vertical = qMax(1, destinationRect.height() / pixmapSize.height()) + 1;
        if (dstPt.y() != destinationRect.y())
            ++vertical;
        ...
        int i = 0;
        for(int y = dstPt.y(); y <= destinationRect.bottom(); y += pixmapSize.height()) {
            Q_ASSERT(i < maxCount);
            qreal x = dstPt.x();
            while (x <= destinationRect.right()) {
                points[i].x = x;
