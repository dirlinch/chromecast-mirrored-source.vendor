...
public:
    ...
    inline void describeSurfaces() {
        static const char *debug_surfaces = getenv("QT_DFB_DEBUG_SURFACES");
        if(!debug_surfaces)
            return;

        qt_graphics_memory = 0;
        ...
        qt_graphics_memory += bytes;
        qDebug("Surfaces(%d): %2.4fk", allocatedSurfaces.size(), float(bytes) / 1024);

        bytes = 0;
        const QWidgetList widgets = QApplication::topLevelWidgets();
        for(QWidgetList::const_iterator it = widgets.begin(); it != widgets.end(); ++it) {
            QWSWindowSurface *windowSurface = (QWSWindowSurface*)(*it)->windowSurface();
            if (windowSurface && windowSurface->key() == QLatin1String("directfb")) {
                QDirectFBWindowSurface *dfbWindowSurface = (QDirectFBWindowSurface*)((*it)->windowSurface());
                QSize sz = dfbWindowSurface->size();
                bytes += sz.height() * sz.width() * 4 * 2;
            }
        }
        qt_graphics_memory += bytes;
        qDebug("WindowSurfaces(%d): %2.4fk", widgets.size(), float(bytes) / 1024);

        bytes = instance->deviceWidth() * instance->deviceHeight() * 4 * 2;
        //qt_graphics_memory += bytes;
        qDebug("ScreenSurfaces(%d): %2.4fk", widgets.size(), float(bytes) / 1024);
    }
