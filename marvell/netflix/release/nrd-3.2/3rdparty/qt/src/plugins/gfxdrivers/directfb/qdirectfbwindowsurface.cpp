...
void QDirectFBWindowSurface::beginPaint(const QRegion &rgn)
{
    if (!engine) {
    ...
    if(win && win->testAttribute(Qt::WA_NoSystemBackground) ) {
        if( IDirectFBSurface *surface = dfbSurface ) {
            DFBRegion clipRegion;
            const QVector<QRect> rects = rgn.rects();
            for (QVector<QRect>::const_iterator it = rects.begin(); it != rects.end(); ++it) {
                clipRegion.x1 = it->x();
                clipRegion.y1 = it->y();
                clipRegion.x2 = it->right();
                clipRegion.y2 = it->bottom();
                surface->SetClip(surface, &clipRegion);
