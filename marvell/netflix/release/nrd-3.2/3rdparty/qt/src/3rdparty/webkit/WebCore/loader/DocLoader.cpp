...
DocLoader::~DocLoader()
{
    ...
    SubCache::ResourceMap resources = m_documentResources.allResources();
    SubCache::ResourceMap::iterator end = resources.end();
    for(SubCache::ResourceMap::iterator it = resources.begin(); it != end; ++it)
...
CachedResource* DocLoader::requestResource(CachedResource::Type type, const String& url, const String& charset, bool isPreload)
{
    ...
    if (cache()->disabled()) {
        if(CachedResource *resource = m_documentResources.resourceForURL(url)) {
            resource->setDocLoader(0);
    ...
    if (resource) {
        ...
        m_documentResources.add(resource);
...
{
#ifndef NDEBUG
    if(CachedResource *tmp = m_documentResources.resourceForURL(resource->url())) 
        ASSERT(tmp == resource);
#endif
    m_documentResources.remove(resource);
}
...
void DocLoader::cancelRequest(Request *request)
{
    m_cache->loader()->cancelRequest(request);
}
