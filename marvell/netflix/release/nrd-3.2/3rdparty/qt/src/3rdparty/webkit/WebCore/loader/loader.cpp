...
void Loader::cancelRequest(WebCore::Request *request)
{
    ...
    HostMap::iterator end = m_hosts.end();
    for (;i != end; ++i) {
        Host *host = i->second.get();
        host->cancelRequest(request);
    }
}
...
void Loader::Host::cancelRequest(Request *requestToCancel)
{
    // Attempt to remove the request from the pending queue
    for (unsigned p = 0; p <= High; p++)
    {
        RequestQueue &requestsPending = m_requestsPending[p];
        ...
        for (RequestQueue::iterator it = requestsPending.begin(); it != end; ++it) {
            Request* request = *it;
            if (request == requestToCancel) {
                //printf("Remove %p\n", requestToCancel);
                cache()->remove(request->cachedResource());
                requestToCancel->docLoader()->decrementRequestCount();
                // WTF::Vector<char> url = requestToCancel->cachedResource()->url().ascii();
                delete request;
            } else
...
#if 0
    // Then attempt to remove a currently processing request
    ...
    for (RequestMap::iterator i = m_requestsLoading.begin(); i != end; ++i) {
        ...
        {
            //printf("Cancel %p\n", requestToCancel);
            // WTF::Vector<char> url = request->cachedResource()->url().ascii();
            i->first.get()->cancel();
            break;
        }
    }
