...
JSValue convertQVariantToValue(ExecState* exec, PassRefPtr<RootObject> root, const QVariant& variant)
{
    ...
    if (type == QMetaType::QObjectStar || type == QMetaType::QWidgetStar) {
        ...
        if (!obj)
            return jsNull();
        return QtInstance::getQtInstance(obj, root, Qt::ValueOwnershipQt)->createRuntimeObject(exec);
    }
            ...
            if (globalobj) {
                ...
                if (exec) {
                    ...
                    if (m_funcObject->inherits(&JSFunction::info)) {
                        ...
                        JSObject* qt_sender = QtInstance::getQtInstance(sender(), ro, Qt::ValueOwnershipQt)->createRuntimeObject(exec);
