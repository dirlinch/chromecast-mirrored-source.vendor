...
void Cache::evict(CachedResource* resource)
{
    ...
    if (resource->inCache()) {
        HashSet<SubCache*>::iterator end = m_subCaches.end();
        for (HashSet<SubCache*>::iterator it = m_subCaches.begin(); it != end; ++it)
            (*it)->remove(resource);
...
void
Cache::addSubCache(SubCache* subCache)
{
        m_subCaches.add(subCache);
}

void
Cache::removeSubCache(SubCache* subCache)
{
    m_subCaches.remove(subCache);
}
...
{
    m_cache->addSubCache(this);
}

SubCache::~SubCache()
{
    m_cache->removeSubCache(this);
}

CachedResource*
SubCache::resourceForURL(const String &url) const
{
    if(!contains(url)) // If it isn't in the subcache then we don't go to the cache for the resource.
        return 0;
    return m_cache->resourceForURL(url);
}
...
SubCache::ResourceMap
SubCache::allResources() const
{
    SubCache::ResourceMap result;
    HashSet<String>::const_iterator end = m_resources.end();
    for (HashSet<String>::const_iterator it = m_resources.begin(); it != end; ++it)
        result.set((*it), m_cache->resourceForURL((*it)));
    return result;
}
