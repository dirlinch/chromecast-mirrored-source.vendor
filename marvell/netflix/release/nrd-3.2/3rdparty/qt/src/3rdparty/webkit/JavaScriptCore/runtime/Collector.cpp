...
// Added by Netflix 2011/05/10
// Enabling ENABLE_FIXED_FREESIZE_GC will cause the "free window" size to be locked to (ALLOCATIONS_PER_COLLECTION *
// FIXED_FREESIZE_FACTOR) in both size and distance-from-currently-used, instead of being (1.25 x double-used-size)
// and double-used-size. It causes garbage collection to happen more often as the free window doesn't become very
// large, but has less overhead on the amount of free memory required after a garbage collect, and works better in
// an environment that triggers manual GCs.
#define ENABLE_FIXED_FREESIZE_GC
#define FIXED_FREESIZE_FACTOR 1
...
void Heap::resizeBlocks()
{
    ...
    const size_t usedCellCount = markedCells();
 #ifdef ENABLE_FIXED_FREESIZE_GC
    const size_t minCellCount = usedCellCount + ALLOCATIONS_PER_COLLECTION;
    const size_t maxCellCount = minCellCount + (ALLOCATIONS_PER_COLLECTION * FIXED_FREESIZE_FACTOR);
#else
    const size_t minCellCount = usedCellCount + max(ALLOCATIONS_PER_COLLECTION, usedCellCount);
    const size_t maxCellCount = 1.25f * minCellCount;
 #endif
    const size_t minBlockCount = (minCellCount + HeapConstants::cellsPerBlock - 1) / HeapConstants::cellsPerBlock;
    const size_t maxBlockCount = (maxCellCount + HeapConstants::cellsPerBlock - 1) / HeapConstants::cellsPerBlock;
