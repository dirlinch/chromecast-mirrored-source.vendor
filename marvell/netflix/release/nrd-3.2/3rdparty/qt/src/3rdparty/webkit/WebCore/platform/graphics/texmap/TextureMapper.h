...
public:
    virtual ~PlatformAnimationClient() {}
...
public:
    ...
    virtual PlatformGraphicsContext* beginPaint(GraphicsContext *gc, const IntRect& dirtyRect) = 0;
    virtual void endPaint() = 0;
    virtual PlatformGraphicsContext* beginPaintMedia()
    {
        return beginPaint(0, IntRect(0, 0, size().width(), size().height()));
    }
    ...
    virtual bool isLocked() { return m_lockCount; }
...
public:
    ...
    virtual void drawRect(const IntRect& target, const Color&color, ColorSpace colorSpaces, const TransformationMatrix& matrix = TransformationMatrix() ) = 0;
