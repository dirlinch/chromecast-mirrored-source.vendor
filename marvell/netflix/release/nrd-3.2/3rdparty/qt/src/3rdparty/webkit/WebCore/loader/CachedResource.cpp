...
void CachedResource::removeClient(CachedResourceClient* client)
{
    ...
    if (m_clients.size() == 1 && m_request) {
        // We test whether we're about to have zero clients instead of
        // after removing our client, as a side-effect of calling cancelRequest
        // is the adjustment of the cache live/dead sizes, which use hasClients()
        // to determine which size to adjust.
        //printf("RemoveClient %p\n", m_request);
        m_cancellingRequest = true; // protect against a delete during cancelRequest()
        m_request->docLoader()->cancelRequest(m_request);
        m_cancellingRequest = false;
    }
