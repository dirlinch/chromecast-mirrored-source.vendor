...
// QtInstance
QtInstance::QtInstance(QObject* o, PassRefPtr<RootObject> rootObject, Qt::ValueOwnership ownership)
    : Instance(rootObject)
    , m_class(0)
    , m_object(o)
    , m_hashkey(o)
...
QtInstance::~QtInstance()
{
    ...
    if (m_object) {
        switch (m_ownership) {
        case Qt::ValueOwnershipQt:
            break;
        case Qt::ValueOwnershipAuto:
            if (m_object->parent())
                break;
            // fall through!
        case Qt::ValueOwnershipScript:
            delete m_object;
            break;
        }
    }
...
PassRefPtr<QtInstance> QtInstance::getQtInstance(QObject* o, PassRefPtr<RootObject> rootObject, Qt::ValueOwnership ownership)
{
    JSLock lock(SilenceAssertionsOnly);
