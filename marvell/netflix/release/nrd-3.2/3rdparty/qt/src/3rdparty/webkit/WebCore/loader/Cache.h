...
public:
    ...
    void addSubCache(SubCache*);
    void removeSubCache(SubCache*);
...
// A SubCache is a subset of the current global Cache. It may not contain all the data in the cache but it will not hold a reference to items
// in the cache so when eviction happens they will get culled from any outstanding SubCache(s). The intention here is to use SubCache as a 
// reference to URL's seen previously (or list of all resources in a document), but once an entity is expired from the Cache it will not continue to
// be referenced (either by resource or URL) elsewhere. 
...
public:
    ...
    typedef HashMap<String, CachedResourceHandle<CachedResource> > ResourceMap;
    ResourceMap allResources() const;

    void add(const String &url);
    inline void add(CachedResource *resource) { add(resource->url()); }

    inline bool contains(const String &url) const { return m_resources.contains(url); }

    CachedResource* resourceForURL(const String &url) const;

    inline void remove(const String &url) { m_resources.remove(url); }
    inline void remove(CachedResource* resource) { remove(resource->url()); }

 private:

    HashSet<String> m_resources;
    Cache *m_cache;
};
