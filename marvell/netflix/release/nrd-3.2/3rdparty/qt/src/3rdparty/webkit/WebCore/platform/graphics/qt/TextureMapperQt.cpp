...
public:
    ...
    virtual PlatformGraphicsContext* beginPaint(GraphicsContext *gc, const IntRect& dirtyRect);
...
public:
    virtual void drawRect(const IntRect& target, const Color&color, ColorSpace colorSpaces, const TransformationMatrix& matrix);
...
bool BitmapTextureQt::isLocked()
{
    if(BitmapTexture::isLocked())
        return true;
    return m_pixmap.paintingActive();
}
...
void BitmapTextureQt::destroy()
{
    ...
    if(!m_pixmap.isNull()) {
#ifdef DEBUG_PIXMAP
        WTFLog(&LogRendering, "PixmapDestroy(%p)", this);
#endif
        m_pixmap = QPixmap();
    }
}
...
#ifdef DEBUG_PIXMAP
        WTFLog(&LogRendering, "PixmapCreate(%p)", this);
#endif
        m_pixmap = QPixmap(size.width(), size.height());
    }
    if (!isOpaque) {
#ifdef DEBUG_PIXMAP
        WTFLog(&LogRendering, "PixmapFill(%p)", this);
#endif
        m_pixmap.fill(Qt::transparent);
    }
}

PlatformGraphicsContext* BitmapTextureQt::beginPaint(GraphicsContext *gc, const IntRect& dirtyRect)
{
    m_painter.begin(&m_pixmap);
    TextureMapperQt::initialize(&m_painter);
    if (gc) {
        m_painter.setRenderHints(gc->platformContext()->renderHints(), true);
        ASSERT(m_painter.renderHints() == gc->platformContext()->renderHints());
    }
#ifdef DEBUG_PIXMAP
    WTFLog(&LogRendering, "BitmapTextureQt::beginPaint %d %d %d %d", dirtyRect.x(), dirtyRect.y(), dirtyRect.width(), dirtyRect.height());
#endif
    m_painter.setClipRect(dirtyRect);
...
void BitmapTextureQt::setContentsToImage(Image* image)
{
    ...
    if (!pixmap || m_pixmap.serialNumber() == pixmap->serialNumber())
        return;
    BitmapTexture::reset(pixmap->size(), !pixmap->hasAlphaChannel());
#ifdef DEBUG_PIXMAP
    WTFLog(&LogRendering, "PixmapAssign(%p)", this);
#endif
    m_pixmap = *pixmap;
}
...
void TextureMapperQt::drawRect(const IntRect& targetRect, const Color&color, ColorSpace colorSpaces, const TransformationMatrix& matrix )
{
...
void TextureMapperQt::drawTexture(const BitmapTexture& aTexture, const IntRect& targetRect, const TransformationMatrix& matrix,
