...
StyleDifference RenderObject::adjustStyleDifference(StyleDifference diff, unsigned contextSensitiveProperties) const
{
    ...
    if (contextSensitiveProperties & ContextSensitivePropertyZIndex) {
        if ((!hasLayer() || !toRenderBoxModelObject(this)->layer()->isComposited()))
...
#if 0 //This is where I can enable the diff code and run through it again!! --SDM
    if(m_style)
        m_style->diff(style.get(), contextSensitiveProperties);
#endif
...
#if 1
    bool same = true;
    for (const FillLayer* currNew = newLayers, *currOld = oldLayers; true; currNew = currNew->next(), currOld = currOld->next()) {
        if(!currNew || !currOld) {
            if(currNew || currOld)
                same = false;
            break;
        }
        if (currOld->image() != currNew->image()) {
            same = false;
            break;
        }
    }
