...
typedef int (*CustomSSLVerifyCallback)(int ok, X509_STORE_CTX *ctx);
Q_NETWORK_EXPORT CustomSSLVerifyCallback sSSLVerifyCallback = 0;
typedef bool (*CustomSSLInitCallback)(SSL*, SSL_CTX*);
Q_NETWORK_EXPORT CustomSSLInitCallback sSSLInitCallback = 0;
...
{
    if(sSSLVerifyCallback)
        ok = (*sSSLVerifyCallback)(ok, ctx);
    if (!ok) {
        if(!ok) { // Store the error and at which depth the error was detected.
...
init_context:
    ...
    if (sSSLInitCallback && !sSSLInitCallback(ssl, ctx)) {
        q->setErrorString(QSslSocket::tr("Custom ssl init callback returned an error"));
        q->setSocketError(QAbstractSocket::UnknownSocketError);
        emit q->error(QAbstractSocket::UnknownSocketError);
        return false;
    }
    ...
    if (configuration.peerVerifyMode == QSslSocket::VerifyNone && !sSSLVerifyCallback) {
