...
//     It does not implement a very good cross-domain verification yet.
    ...
    {
        static int insecure_js_sees_secure_cookies = -1;
        if(insecure_js_sees_secure_cookies == -1) {
            if(char *e = getenv("INSECURE_JS_SEES_SECURE_COOKIES"))
                insecure_js_sees_secure_cookies = atoi(e);
            else
                insecure_js_sees_secure_cookies = 0;
        }
        if(insecure_js_sees_secure_cookies == 1)
            isEncrypted = true;
    }
