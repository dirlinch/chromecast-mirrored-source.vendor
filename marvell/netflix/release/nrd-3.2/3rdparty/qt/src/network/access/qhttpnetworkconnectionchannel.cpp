...
void QHttpNetworkConnectionChannel::close()
{
    ...
    enterState(QHttpNetworkConnectionChannel::IdleState);
}
...
//        socket->flush();
        ...
        if (uploadByteDevice) {
            ...
            enterState(QHttpNetworkConnectionChannel::WritingState);  // start writing data
            sendRequest(); //recurse
        } else {
            enterState(QHttpNetworkConnectionChannel::WaitingState); // now wait for response
            sendRequest(); //recurse
        }
...
bool QHttpNetworkConnectionChannel::ensureConnection()
{
    ...
    if (socketState != QAbstractSocket::ConnectedState) {
        ...
        enterState(QHttpNetworkConnectionChannel::ConnectingState);
...
void QHttpNetworkConnectionChannel::allDone()
{
    ...
    this->enterState(QHttpNetworkConnectionChannel::IdleState);
    ...
    if (!alreadyPipelinedRequests.isEmpty()) {
        if (resendCurrent || connectionCloseEnabled || socket->state() != QAbstractSocket::ConnectedState) {
            ...
            enterState(QHttpNetworkConnectionChannel::ReadingState);
            resendCurrent = false;
...
void
QHttpNetworkConnectionChannel::enterState(ChannelState s)
{
    state = s;
    if(waitingTimer != -1) {
...
#if 1
    if(state == WaitingState || state == ConnectingState) {
        qDebug("Starting timeout for qhttpnetworkconnectionchannel [%s]", request.url().toString().toLatin1().constData());
        waitingTimer = startTimer(60000);
    }
#endif
}

void
QHttpNetworkConnectionChannel::timerEvent(QTimerEvent *e)
{
    if(e->type() == QEvent::Timer) {
        QTimerEvent *te = (QTimerEvent*)e;
        if(te->timerId() == waitingTimer) {
            killTimer(waitingTimer);
            waitingTimer = -1;
            qCritical("Closing request due to qhttpnetworkconnectionchannel timeout! [%s]", request.url().toString().toLatin1().constData());
            _q_error(QAbstractSocket::SocketTimeoutError);
            socket->abort();
            return;
        }
    }
