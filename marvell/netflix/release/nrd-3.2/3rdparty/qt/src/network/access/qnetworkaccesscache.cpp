...
void QNetworkAccessCache::linkEntry(const QByteArray &key)
{
	...
	// enforce cache size limit, destroy this node instead of linking it
    if (hash.size() > CachedNodeLimit) {
        Q_ASSERT(!oldest && !newest);
        node->object->dispose();
        hash.remove(key);
        return;
    }
...
void QNetworkAccessCache::addEntry(const QByteArray &key, CacheableObject *entry)
{
    ...
    bool modified = false;
    while ((hash.size() > CachedNodeLimit) && oldest) {
        Node * n = oldest;
        Q_ASSERT(n->key != key);
        Q_ASSERT(n->useCount == 0);
        n->object->dispose();
        unlinkEntry(n->key);
        hash.remove(n->key);
        modified = true;
    ...
    if (hash.size() > CachedNodeLimit)
        qWarning("QNetworkAccessCache::addEntry: over cache budget: %d\n", hash.size() - CachedNodeLimit);
    if (modified)
        updateTimer();
