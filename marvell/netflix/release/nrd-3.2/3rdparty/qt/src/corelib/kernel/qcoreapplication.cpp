...
bool QCoreApplication::notifyInternal(QObject *receiver, QEvent *event)
{
    ...
    static int cnt = 0;
    int foo = cnt++;
    QTime _watch; _watch.start();
    qDebug() << foo << "ProcessEvent" << event->type() << receiver->metaObject()->className();
#endif
    ...
    //qDebug() << foo << "~ProcessEvent" << event->type() << _watch.elapsed();
    return returnValue;
}
...
#if defined(Q_OS_WIN) || defined(Q_OS_SYMBIAN) || defined(Q_OS_PS3)
            QLatin1Char pathSep(';');
#else
            QLatin1Char pathSep(':');
#endif
