...
#if defined(Q_CC_SNC)
    do {
        originalValue = __lwarx(&_q_value);
		newValue = originalValue + 1;
    } while (!__stwcx((volatile int *)&_q_value, newValue));
#else
...
#if (defined(__64BIT__) || defined(__powerpc64__) || defined(__ppc64__)) && !defined(__PPU__)
#  define LPARX "ldarx"
#  define STPCX "stdcx."
#else
#  define LPARX "lwarx"
...
Q_INLINE_TEMPLATE bool QBasicAtomicPointer<T>::testAndSetRelaxed(T *expectedValue, T *newValue)
{
	...
	do {
        originalValue = (T *)__lwarx(&_q_value);
		if (originalValue != expectedValue)
			return false;
    } while (!__stwcx((volatile int *)&_q_value, (unsigned int)newValue));
	return true;
#else
